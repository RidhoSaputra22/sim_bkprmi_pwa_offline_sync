            ...
            <form action="{{ route('tpa.santri.index') }}" method="GET" class="flex flex-wrap gap-4">
                <div class="form-control flex-1 min-w-[200px]">
                    <input type="text" name="search" value="{{ request('search') }}"
                           class="input input-bordered" placeholder="Cari nama santri...">
                </div>
                ...
                <table class="table table-zebra">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-sm text-base-content/60">{{ $santri->person?->nik ?? '-' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <span class="badge badge-{{ $santri->person?->gender?->value == 'L' ? 'info' : 'secondary' }} badge-sm">
                                    {{ $santri->person?->gender?->value ?? '-' }}
                                </span>
                            </td>
                            <td>{{ $santri->jenjang?->getLabel() ?? '-' }}</td>
                            <td>{{ $santri->kelas?->getLabel() ?? '-' }}</td>
                            <td>
                            ...
                            <td>
                                @if($santri->guardians->first())
                                <div class="text-sm">
                                    {{ $santri->guardians->first()->person?->full_name ?? '-' }}
                                    <span class="text-base-content/60">({{ $santri->guardians->first()->pivot?->hubungan?->getLabel() ?? '-' }})</span>
                                </div>
                                @else
                            ...
                            <td>
                                <div class="flex gap-1">
                                    <a href="{{ route('tpa.santri.show', $santri) }}" class="btn btn-ghost btn-xs" title="Lihat Detail">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                        </svg>
                                    </a>
                                    <a href="{{ route('tpa.santri.edit', $santri) }}" class="btn btn-ghost btn-xs" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                        </svg>
                                    </a>
                                    <form action="{{ route('tpa.santri.destroy', $santri) }}" method="POST" class="inline"
