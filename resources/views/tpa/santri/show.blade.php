                ...
                <div class="card-body">
                    ...
                    <div class="grid grid-cols-2 gap-4 mt-4">
                        ...
                        <div>
                            ...
                            <p>{{ $santri->person?->child_order ?? '-' }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-base-content/60">Jumlah Saudara</p>
                            <p>{{ $santri->person?->siblings_count ?? '-' }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="space-y-6">
            ...
            <div class="card bg-base-100 shadow">
                <div class="card-body items-center text-center">
                    ...
                    <div class="flex gap-2 mt-2">
                        <span class="badge badge-{{ $santri->person?->gender?->value == 'L' ? 'info' : 'secondary' }}">
            ...
            <div class="card bg-base-100 shadow">
                <div class="card-body">
                    ...
                    @forelse($santri->guardians as $guardian)
                    <div class="mt-4 p-4 bg-base-200 rounded-lg">
                        <div class="flex items-center gap-3 mb-3">
                            <div class="avatar placeholder">
                                <div class="bg-neutral text-neutral-content rounded-full w-10">
                                    <span>{{ substr($guardian->person?->full_name ?? 'W', 0, 1) }}</span>
                                </div>
                            </div>
                            <div>
                                <p class="font-medium">{{ $guardian->person?->full_name ?? '-' }}</p>
                                <p class="text-sm text-base-content/60">{{ $guardian->pivot?->hubungan?->getLabel() ?? '-' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span class="font-mono">{{ $guardian->person?->nik ?? '-' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-base-content/60">No. HP</span>
                                <span>{{ $guardian->person?->phone ?? '-' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-base-content/60">Pekerjaan</span>
                                <span>{{ $guardian->pekerjaan?->getLabel() ?? '-' }}</span>
                            </div>
                        </div>
                    </div>
                    @empty
