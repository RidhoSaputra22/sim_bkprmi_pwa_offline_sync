<x-layouts.admin title="Edit Santri">
    ...
    <form method="POST" action="{{ route('admin.santri.update', $santri) }}" class="space-y-6" x-data="santriForm()">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <x-ui.input
                        ...
                        :required="true"
                        min="1"
                        :value="old('child_order', $santri->child_order ?? 1)"
                    />
                    ...
                    <x-ui.input
                        ...
                        :required="true"
                        min="0"
                        :value="old('siblings_count', $santri->siblings_count ?? 0)"
                    />
                </div>
            </div>
        </x-ui.card>
        ...
        <x-ui.card title="NAMA PENGAMPU / WALI SANTRI" subtitle="Data wali yang bertanggung jawab atas santri">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <x-ui.input
                    ...
                    maxlength="16"
                    pattern="[0-9]{16}"
                    :value="old('wali_nik', $guardianPerson->nik ?? '')"
                />
                ...
                <x-ui.input
                    ...
                    :required="true"
                    :value="old('wali_nama', $guardianPerson->full_name ?? '')"
                />
                ...
                <x-ui.input
                    ...
                    :required="true"
                    :value="old('wali_tempat_lahir', $guardianPerson->birth_place ?? '')"
                />
                ...
                @php
                    $hubunganOpts = collect($hubunganOptions)->mapWithKeys(fn($h) => [$h->value => $h->getLabel()])->toArray();
                @endphp
                <x-ui.select
                    name="wali_hubungan"
                    label="Hubungan Dengan Santri"
                    :options="$hubunganOpts"
                    :required="true"
                    :selected="old('wali_hubungan', $guardianSantri->hubungan ?? '')"
                />
                ...
                <x-ui.select
                    ...
                    :required="true"
                    :selected="old('wali_pendidikan', $guardian->pendidikan_terakhir ?? '')"
                />
        ...
        <x-ui.card title="STATUS SANTRI" subtitle="Status pendidikan santri di TPA/TPQ">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                @php
                    $jenjangOpts = collect($jenjangOptions)->mapWithKeys(fn($j) => [$j->value => $j->getLabel()])->toArray();
    ...
    <script>
        function santriForm() {
            return {
                provinceId: '{{ old('province_id', $santri->village?->district?->city?->province_id ?? '') }}',
                cityId: '{{ old('city_id', $santri->village?->district?->city_id ?? '') }}',
