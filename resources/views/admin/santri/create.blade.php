<x-layouts.admin title="Tambah Santri">
    <x-slot:breadcrumb>
        <ul>
    ...
    >
        ...
        <x-ui.card title="NAMA PENGAMPU / WALI SANTRI" subtitle="Data wali yang bertanggung jawab atas santri">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        @foreach($genderOptions as $gender)
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input
                                    ...
                                    value="{{ $gender->value }}"
                                    class="radio radio-primary"
                                    {{ old('wali_gender') == $gender->value ? 'checked' : '' }}
                                    required
                                />
                                <span>{{ $gender->getLabel() }}</span>
                            </label>
                ...
                @php
                    $hubunganOptions = collect($hubunganOptions)->mapWithKeys(fn($h) => [$h->value => $h->getLabel()])->toArray();
                @endphp
                <x-ui.select
                    name="wali_hubungan"
                    ...
                    :required="true"
                    :selected="old('wali_hubungan')"
                />

                @php
                    $pendidikanOptions = collect($pendidikanOptions)->mapWithKeys(fn($p) => [$p->value => $p->getLabel()])->toArray();
                @endphp
                <x-ui.select
                    ...
                    :required="true"
                    :selected="old('wali_pendidikan')"
                />

                @php
                    $pekerjaanOptions = collect($pekerjaanOptions)->mapWithKeys(fn($p) => [$p->value => $p->getLabel()])->toArray();
                @endphp
                <x-ui.select
                    name="wali_pekerjaan"
                    label="Pekerjaan"
        ...
        <x-ui.card title="STATUS SANTRI" subtitle="Status pendidikan santri di TPA/TPQ">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                @php
                    $jenjangOptions = collect($jenjangOptions)->mapWithKeys(fn($j) => [$j->value => $j->getLabel()])->toArray();
                    $kelasOptions = collect($kelasOptions)->mapWithKeys(fn($k) => [$k->value => $k->getLabel()])->toArray();
                ...
                <x-ui.select
                    ...
                    :options="$statusOpts"
                    :required="true"
                    :selected="old('status_santri', 'aktif')"
                />
            </div>
        </x-ui.card>
