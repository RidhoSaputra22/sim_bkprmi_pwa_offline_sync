...
### Sample Migration untuk UserRole
```php
Schema::create('user_roles', function (Blueprint $table) {
    $table->id();
    $table->foreignId('user_id')->constrained()->cascadeOnDelete();
    $table->string('role'); // member, anggota, admin, etc
    $table->timestamps();
});
```
...
public function test_member_can_download_report()
{
    $user = User::factory()
        ->has(UserRole::factory()->state(['role' => 'member']))
        ->create();
...
- `README_MEMBER_IMPLEMENTATION.md` - File ini
...
Implementasi berdasarkan Use Case Diagram Sistem Informasi BKPRMI

## 📄 License

Sesuai dengan license project Laravel BKPRMI
